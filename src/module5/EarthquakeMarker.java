...
public abstract class EarthquakeMarker extends CommonMarker
{
	...
	public void showTitle(PGraphics pg, float x, float y)
	{
		...
		String age = getStringProperty("age");
		String magnitude = "Magnitude: " + getMagnitude();
		String depth = "Depth: " + getDepth();
		String distanceTo = "Distance to: " + getDistanceTo(getLocation()) + " km";
		
		pg.pushStyle();
		
		pg.fill(255, 255, 255);
		pg.rectMode(PConstants.CORNER);
		pg.rect(x, y-39, Math.max(pg.textWidth(title), pg.textWidth(magnitude)) + 6, 120);
		pg.fill(0, 0, 0);
		pg.textAlign(PConstants.LEFT, PConstants.TOP);
		pg.text(title, x+3, y-33);
		pg.text(age, x+3, y-18);
		pg.text(magnitude, x+3, y-3);
		pg.text(depth, x+3, y+12);
		pg.text(distanceTo, x+3, y+27);
